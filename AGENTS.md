...
Agentic intraday trading desktop app for Angel One SmartAPI. An Electron + React
...
- Strategy signal functions are pure (`DataFrame -> signals`); test them with
  ...
  hit external APIs or the network in unit tests.
- Add or update tests for any change to strategy or trading logic.

## Safety — non-negotiable

- **Never** request, print, hardcode, commit, or modify real SmartAPI
  credentials (API Key, Client Code, MPIN, TOTP Secret). Credentials are
  configured by the user through the app's Settings / Login modal and stored
  encrypted in `~/.smartapi-agentic-trading/`.
