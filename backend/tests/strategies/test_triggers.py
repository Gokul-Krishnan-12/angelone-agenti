...
class TestEMACrossover:
    def test_buy_on_golden_cross_with_volume(self):
        ...
        assert sig["direction"] == "BUY"
        # Confidence uses new formula: 68 + (vol_ratio - 1.5) * 8, capped at 100.
        # The crossover bar gets ≥ 1.5× volume → confidence > 68.
        assert sig["confidence"] >= 82

    def test_no_signal_without_volume_confirmation(self):
        # Same golden cross, but the breakout bar has below-average volume (< 1.5x).
        closes = list(np.linspace(120, 100, 26)) + list(np.linspace(100, 112, 8))
