try {
  ...
  electron.contextBridge.exposeInMainWorld('electronAPI', {
    ...
    auth: {
      login: (creds: any, apiSecret?: string) =>
        electron.ipcRenderer.invoke(
          channels.AUTH_LOGIN,
          typeof creds === 'string' ? { apiKey: creds, apiSecret } : creds
        ),
    ...
    orders: {
      ...
      getAll: (params?: any) => electron.ipcRenderer.invoke(channels.ORDERS_GET_ALL, params),
      getTrades: () => electron.ipcRenderer.invoke(channels.ORDERS_GET_TRADES),
    },
    portfolio: {
      positions: (params?: any) => electron.ipcRenderer.invoke(channels.PORTFOLIO_POSITIONS, params),
      holdings: () => electron.ipcRenderer.invoke(channels.PORTFOLIO_HOLDINGS),
      margins: (params?: any) => electron.ipcRenderer.invoke(channels.PORTFOLIO_MARGINS, params),
    },
    market: {
    ...
    agent: {
      start: (params?: any) => electron.ipcRenderer.invoke(channels.AGENT_START, params),
